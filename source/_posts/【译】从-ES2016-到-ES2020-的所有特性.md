...
##### 反向断言
> 使用反向断言可以确保匹配之前或者之后没有其他匹配。反向断言的语法表示为 `(?<=...)` ，例如：匹配一个美元数值且不包含美元符号可以这样写 `/(?<=$)\d+(\.\d*)?/`，这个表达式会匹配 `$10.53` 并返回 `10.53`，而并不会匹配 `€10.53`。而 `(?<!...)` 匹配的规则正相反，它会匹配不存在表达式中的匹配项，例如 `/(?<!$)\d+(?:\.\d*)/` 不会匹配 `$10.53`，但是会匹配 `€10.53`。
